...
{
    /*Ambient */
    ambient = light.ambient * texture(material.specular_map, TexCoords).rgb;   /*use specular texture */
    
    /*Diffuse */
    vec3 norm = normalize(Normal);
    lightDir = get_light_dir();
    float diff = max(dot(norm, lightDir), 0.0);
    diffuse = light.diffuse * diff * texture(material.specular_map, TexCoords).rgb;   /*use specular texture */
    
    /*Specular */
    vec3 viewDir = normalize(viewPos - FragPos);
    vec3 reflectDir = reflect(-lightDir, norm);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
    specular = light.specular * spec * texture(material.specular_map, TexCoords).rgb;    /*use specular texture */
    
    /*Emission */
    if (texture(material.specular_map, TexCoords).r == 0.0)   /*rough check for blackbox inside spec texture */
    {
        /*apply emission texture */
        emission = texture(material.emissive_map, TexCoords).rgb;
        
        /*some extra fun stuff with "time uniform" */
        emission = texture(material.emissive_map, TexCoords + vec2(0.0,time)).rgb;   /*moving */
