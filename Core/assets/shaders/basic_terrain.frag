...
const float level_4 = 140.0;
const float level_5 = 190.0;
...
void main()
{
    ...
    vec4 color_5 = texture(layer_5, fs_in.TexCoords * (uv_scale * 7.5)) * texture(noise, fs_in.TexCoords * (uv_scale));

	vec4 result = vec4(vec3(0.0), 1.0);

    if (Height < level_1)
    {
        result = (water + 0.01) + color_1;
    }    
