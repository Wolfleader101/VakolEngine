...
void main()
{
    vec4 lighting = vec4(texture(light_map, TexCoord * vec2(1.0, 1.0)).rrr, 1.0);

	vec4 color_1 = texture(layer_1,   TexCoord * (uv_scale * 6));
	vec4 color_2 = texture(layer_2,   TexCoord * (uv_scale * 2));
	vec4 color_3 = texture(layer_3,   TexCoord * (uv_scale * 10));
	vec4 color_4 = texture(layer_4,   TexCoord * (uv_scale * 4));
	vec4 color_5 = texture(layer_5,   TexCoord * (uv_scale * 4));
	vec4 color_6 = texture(layer_6,   TexCoord * (uv_scale * 15));
	vec4 color_7 = texture(layer_7,   TexCoord * (uv_scale * 4));
	vec4 color_8 = texture(layer_8,   TexCoord * (uv_scale * 25));
	vec4 color_9 = texture(layer_9,   TexCoord * (uv_scale * 4));
	vec4 color_10 = texture(layer_10, TexCoord * (uv_scale * 1.5));
