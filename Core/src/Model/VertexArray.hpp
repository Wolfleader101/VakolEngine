...
namespace Vakol::Model {
    ...
    enum class DRAW_MODE { DEFAULT, INSTANCED, STRIPS, PATCHES };

    // draw with vertices only (arrays), or using indices (elements)
    enum class DRAW_TYPE { ARRAYS, ELEMENTS };
    ...
    class VertexArray {
       public:
        ...
        /// @param stride byte offset between each consecutive vertex *just keep this at the size* (Our data is not
        /// tightly packed).
        ...
        /// Example 2: Normal (aNormal) = (void*)offset of(Vertex, normal) *if using Vertex struct* OR (void*)(3 *
        /// sizeof(float))
