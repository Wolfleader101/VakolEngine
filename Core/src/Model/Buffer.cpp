...
/******************************************************************************************************************/
	...
	FrameBuffer::FrameBuffer(const ATTACHMENT& attachment, const bool depth)
    {
        ...
        SetTextureAttachment(attachment.color.attachment, GL_TEXTURE_2D, 0,
							 attachment.color.internalformat, attachment.color.width, attachment.color.height, 
							 attachment.color.format, GL_UNSIGNED_BYTE, nullptr);

        if (depth)
            SetDepthAttachment(attachment.depth.attachment, attachment.depth.internalformat, attachment.color.width, attachment.color.height);
    ...
    void FrameBuffer::ClearColor(const float r, const float g, const float b, const float a) const { glClearColor(r, g, b, a); }
    ...
    void FrameBuffer::RenderBufferStorage(const int internalformat, const int width, const int height) const { glRenderbufferStorage(GL_RENDERBUFFER, internalformat, width, height); }
    void FrameBuffer::AttachRenderBuffer(const unsigned int attachment) const { glFramebufferRenderbuffer(GL_FRAMEBUFFER, attachment, GL_RENDERBUFFER, this->m_RenderBufferID); }

    void FrameBuffer::SetTextureAttachment(const unsigned int attachment, const unsigned int target, const int level, const int internalformat, const int width, const int height, const unsigned int format, const unsigned int type, const void* data)
    {
    ...
    void FrameBuffer::SetDepthAttachment(const unsigned int attachment, const int internalformat, const int width, const int height)
    {
        CreateRenderBuffer();
        BindRenderBuffer();
