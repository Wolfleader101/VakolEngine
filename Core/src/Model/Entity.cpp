...
namespace Vakol::Model {
    ...
    uint32_t Entity::GetHandle() const { return uint32_t(m_entityHandle); }

    void Entity::SetHandle(uint32_t handle) { m_entityHandle = (entt::entity)handle; }

    Entity::operator uint32_t() const { return (uint32_t)m_entityHandle; }

    Entity::operator bool() const { return m_entityHandle != entt::null && m_EntityList != nullptr; }
