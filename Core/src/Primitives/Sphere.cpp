...
namespace Vakol
{
    ...
    Sphere::Sphere(Components::Transform inputTransform, double inputRadius, unsigned inputStacks,
                   unsigned inputSectors, std::string inputName, std::string inputID)
    {
    ...
    void Sphere::GenerateData(Components::Transform inputTransform, double inputRadius, unsigned inputStacks,
                              unsigned inputSectors)
    {
        ...
        for (int i = 0; i <= inputStacks; ++i)
        {
            ...
            z = (inputRadius * sin(stackAngle)) +
                inputTransform.pos.z; // r * sin(u) (Add position to move sphere to point)
            ...
            for (int j = 0; j <= inputSectors; ++j)
            {
                ...
                x = xy * (cos(sectorAngle)) +
                    inputTransform.pos.x; // r * cos(u) * cos(v) (Add position to move sphere to point)
                y = xy * (sin(sectorAngle)) +
                    inputTransform.pos.y; // r * cos(u) * sin(v) (Add position to move sphere to point)
