...
unsigned int LoadGLTexture(std::vector<const char*>&& faces, const bool gamma, const bool flip)
{
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        const GLint internal_format = (channels == 1)  ? GL_R8
                                      : (channels > 3) ? (gamma) ? GL_SRGB8_ALPHA8 : GL_RGBA8
                                      : (gamma)        ? GL_SRGB8
                                                       : GL_RGB8;
        const GLenum data_format = (channels == 1) ? GL_RED : (channels > 3) ? GL_RGBA : GL_RGB;

        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, internal_format, width, height, 0, data_format,
