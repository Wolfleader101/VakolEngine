...
float Lerp(const float a, const float b, const float t) { return (1.0f - t) * a + b * t; }

/// return a fraction 't' (between 0 and 1) based on the given value
float InverseLerp(const float a, const float b, const float v) { return (v - a) / (b - a); }
